...
| Mode | full | `--quick`, `--regression .kstack/qa-reports/baseline.json` |
| Output dir | `.kstack/qa-reports/` | `Output to /tmp/qa` |
| Scope | Full app (or diff-scoped) | `Focus on the billing page` |
| Auth | None | `Sign in to user@example.com`, `Load auth state from state.json` |
...
2. **Identify affected pages/routes** from the changed files:
   ...
   - API endpoints → test them directly with `$B open` + JS evaluation
   - Static pages (markdown, HTML) → navigate to them directly

3. **Detect the running app** — check common local dev ports:
   ```bash
   $B open http://localhost:3000 2>/dev/null && echo "Found app on :3000" || \
   $B open http://localhost:4000 2>/dev/null && echo "Found app on :4000" || \
   $B open http://localhost:8080 2>/dev/null && echo "Found app on :8080"
   ```
...
$B click @e5                      # submit
$B diff snapshot                  # verify login succeeded
```

**If the user provided an auth state file:**

```bash
$B state load auth-state.json
$B open <target-url>
```
...
```bash
$B open <target-url>
$B screenshot --annotate "$REPORT_DIR/screenshots/initial.png"
$B snapshot -i                       # map interactive elements
$B console                           # any errors on landing?
```
...
**For SPAs:** Use `snapshot -i` to find nav elements (buttons, menu items) for client-side navigation.
...
```bash
$B open <page-url>
$B screenshot --annotate "$REPORT_DIR/screenshots/page-name.png"
$B console
```
...
7. **Responsiveness** — Check mobile viewport if relevant:
   ```bash
   $B set viewport 375 812
   $B screenshot "$REPORT_DIR/screenshots/page-mobile.png"
   $B set viewport 1280 720
   ```
...
```bash
$B screenshot --annotate "$REPORT_DIR/screenshots/issue-002.png"
```
...
- Monitor network requests for `_next/data` 404s indicating broken data fetching
...
- Use `snapshot -i` for navigation — client-side routes

...
kstack is powerful with one Claude Code session. It is transformative with ten.
...
Each workspace gets its own isolated browser instance automatically — separate Chromium process, cookies, tabs, and logs stored in `.kstack/` inside each project root. No port collisions, no shared state, no configuration needed. `/browse` and `/qa` sessions never interfere with each other, even across ten parallel workspaces.
...
**Requirements:** [Claude Code](https://docs.anthropic.com/en/docs/claude-code), [Git](https://git-scm.com/), [Node.js](https://nodejs.org/) (for agent-browser install). `/browse` uses a native Rust binary — works on macOS and Linux (x64 and arm64).
...
- Skill files (Markdown prompts) in `~/.claude/skills/kstack/` (or `.claude/skills/kstack/` for project installs)
- Symlinks at `~/.claude/skills/browse`, `~/.claude/skills/qa`, `~/.claude/skills/review`, etc. pointing into the kstack directory
- agent-browser CLI (installed globally via npm, ~7MB)
...
Created by [Kagan Demirhindi](https://kagan.dev) ([LinkedIn](https://www.linkedin.com/in/kagan-demirhindi/)).

I built kstack because I do not want AI coding tools stuck in one mushy mode.
...
**Setup:** Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, kstack picks up its comments automatically. No additional configuration.
...
kstack solves this. `/review` and `/ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
It also learns. Every false positive you confirm gets saved to `~/.kstack/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
...
It is powered by [agent-browser](https://github.com/vercel-labs/agent-browser), a native Rust headless browser CLI by Vercel Labs. First call starts the browser (~2s). Every call after that: ~100ms. The browser stays running between commands, so cookies, tabs, and localStorage carry over.
...
Run `cd ~/.claude/skills/kstack && ./setup` (or `cd .claude/skills/kstack && ./setup` for project installs). This rebuilds symlinks so Claude can discover the skills.

**`/browse` fails or agent-browser not found?**

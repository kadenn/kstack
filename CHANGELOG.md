...
### Added
- **Daily update check** — all 9 skills now check for new versions once per day via `bin/kstack-update-check` (pure bash, <5ms cached). Prompts user via AskUserQuestion with option to upgrade or defer 24h.
- **`/kstack-upgrade` skill** — standalone upgrade command that detects install type (global-git, local-git, vendored), upgrades, and shows a "What's New" summary from CHANGELOG
- **"Just upgraded" confirmation** — after upgrading, the next skill invocation shows "Running kstack v{new} (just updated!)" via `~/.kstack/just-upgraded-from` marker
- **`AskUserQuestion` added to 5 skills** — kstack (root), browse, qa, retro, setup-browser-cookies now have AskUserQuestion in allowed-tools for upgrade prompts
- **`Bash` added to plan-eng-review** — enables the update check preamble to run in plan review sessions
- `browse/test/kstack-update-check.test.ts` — 10 test cases covering all script branch paths with `KSTACK_REMOTE_URL` env var for test isolation
- `TODOS.md` for tracking deferred work

### Changed
- **Version check is now one system** — removed SHA-based `checkVersion()` from `browse/src/find-browse.ts` (~120 lines deleted) and `browse/test/find-browse.test.ts` (~100 lines deleted). Replaced by `bin/kstack-update-check` bash script using semver VERSION comparison with 24h cache.
...
- Legacy `/tmp/kstack-latest-version` cache file (cleaned up by `setup` script)
...
- **Update check exit code 1 misleading agents** — `[ -n "$_UPD" ] && echo "$_UPD"` returned exit code 1 when no update available, causing agents to think kstack was broken. Added `|| true`.
...
- Setup block now checks workspace-local path first (for development), then falls back to global `~/.claude/skills/kstack/browse/dist/browse`
...
- *(removed in 0.4.1)* skill:check, dev:skill, gen:skill-docs, test:eval — replaced by direct SKILL.md editing
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.claude/skills/kstack` fallback from `resolveServerScript()`
- Crash log path references updated from `/tmp/` to `.kstack/`
...
- **Project-local browse state** — state file, logs, and all server state now live in `.kstack/` inside the project root (detected via `git rev-parse --show-toplevel`). No more `/tmp` state files.
...
### Changed
- State file location: `.kstack/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.kstack/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/kstack/browse/src/server.ts`
- `DEVELOPING_KSTACK.md` (renamed to CONTRIBUTING.md)
